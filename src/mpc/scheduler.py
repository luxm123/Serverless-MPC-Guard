...
class Scheduler:
    ...
    def decide_control_input(self, priority, wcp_constraints, system_state, ref_latency=None):
        ...
        if priority >= prio_high_thr:
            if price > price_high and (ub_latency > self.slo_limit or composite > 0.0):
                should_shed = True
            ...
            if (ub_latency > relaxed_limit or composite > 0.0) and price > price_med:
                should_shed = True
                degrade_plan = "store_to_sqs_recovery"
        else:
            price_low = float(system_state.get('sched_price_low', 10.0))
            if (ub_latency > self.slo_limit or composite > 0.0) and price > price_low:
                should_shed = True
