...
def plot_time_series(mpc_csv, baseline_csv, output_dir, trial_suffix=''):
    ...
    try:
        ...
        # Bin data by second on each dataframe so plotting with df_mpc/df_base works
        def _add_time_bin(df):
            tc = 'arrival_timestamp' if 'arrival_timestamp' in df.columns else 'timestamp'
            df['TimeBin'] = df[tc].astype(int)
            return df
        df_mpc = _add_time_bin(df_mpc)
        df_base = _add_time_bin(df_base)
        ...
        # 2. Allocation (MPC Only)
        sns.lineplot(data=df_mpc, x='TimeBin', y='alloc', estimator='mean', errorbar=None, ax=axes[1], color='g')
        axes[1].set_title(f'MPC Resource Allocation Adaptation {trial_suffix}')
        axes[1].set_ylabel('Allocation (0.4-1.0)')
